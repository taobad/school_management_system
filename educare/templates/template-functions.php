			...
			<div class="content">
				...
				foreach ($requred_title as $key => $value) {
					if($key == 'Name' || $key == 'Roll_No' || $key == 'Regi_No') {
						...
						echo '<input type="text" name="default['.esc_attr($key).']" value="'.esc_attr($field_value).'" placeholder="Inter '.esc_attr($value).'">';
					} else {
						$i++;
						if($i % 2 == 1) {
						...
						if ($key == 'auto_fill') {
							echo '<input type="submit" name="search" value="Search" class="educare_button crud-forms"></input>';
						} else {
							echo '<div>';
							// echo '<p>Select '.esc_html($value).':</p>';
 ...
 * @return void
 */
function educare_get_extra_field() {
	$select = array (
		'table' => 'settings',     // SELECT * FROM $select
		'row' => 'list',           // WHERE $row (need to compare with value)
	...
	if ($results) {
		foreach ($results as $details) {
			$type = strtok($details, ' ');
			$display = substr(strstr($details, ' '), 1);
			...
			<div class="wrap-input">
				<span class="input-for">'.esc_html($display).':</span>
				<label for="'.esc_attr($display).'" class="labels" id="'.esc_attr($display).'"></label>
				<input type="'.esc_attr($type).'" name="Details['.esc_attr($name).']" class="fields" value="'.esc_attr($value).'" placeholder="Inter '.esc_attr($display).'">
 ...
 */
function educare_grade_sheet_th($terms, $front = false) {
	foreach ($terms as $term => $termInfo) {
		...
		echo '<th>'. esc_html($title) . wp_kses_post( $subtitle ).'</th>';
	}
}
		...
		<table class="grade_sheet list">
			...
			<tbody>
				...
				if ($subjects) {
					...
					// wen don't need to additon optional field. beacuse $default_terms has 3 incliding grade_sheed!
  ...
  if (isset($_POST['select_subject'])) {
	...
	if ($roles == 'results') {
		...
		if ($auto_results == 'checked') {
			echo educare_guide_for("Please <a href='/wp-admin/admin.php?page=educare-settings&menu=Results_System' target='_blank'>Disable Auto Results</a> system from Educare settings to manually set <b>".esc_html( $results_card["result"] )."</b> or <b>".esc_html( $results_card["gpa"] )."</b>");
		}
	...
	<br>
	<!-- Begin Group -->
	<?php echo educare_guide_for('Click here to <a href="/wp-admin/admin.php?page=educare-management&Group" target="_blank">Add More Group</a>.');?>
	...
	<div class="select">
		...
		<div>
			...
			<select id="publish" name="default[Status]">
				<option value="publish" <?php selected( 'publish', esc_attr($Status) ) ;?>>Publish</option>
				<option value="scheduled" <?php selected( 'scheduled', esc_attr($Status) ) ;?>>Scheduled</option>
			</select>
		</div>
			...
			<div class="content">
				...
				foreach ($requred_title as $key => $value) {
					...
					if($key == 'Name' || $key == 'Roll_No' || $key == 'Regi_No') {
						...
						echo '<p>'.esc_html($value).'</p>';
						echo '<input type="text" name="default['.esc_attr($key).']" value="'.esc_attr($field_value).'" placeholder="Enter '.esc_attr($value).'">';
					} else {
						$i++;
						if($i % 2 == 1) {
							echo '<div class="select">';
						...
						if ($key == 'auto_fill') {
							...
							echo '<div><div name="auto_fill" class="educare_button crud-forms" style="margin: 0px;" title="Get data/details from specific student profiles. For this, you need to fill roll no, regi no, class and year.">Auto Fill</div></div>';
						} else {
							echo '<div>';
							// echo '<p>Select '.esc_html($value).':</p>';
							echo '<select id="'.esc_attr($key).'" name="default['.esc_attr($key).']">';
				...
				if ($roles != 'results') {
					...
					if ($user_profiles == 'checked') {
						...
						echo educare_guide_for('Please carefully fill login information. If you want to disable login system, just <a href="/wp-admin/admin.php?page=educare-settings&menu=Security" target="_blank">Click here</a> and uncheck <b>User Profiles</b> options.');
						// Username
						echo '<p>Username</p>';
						...
						echo '
						<span class="button updatePasswordBtn" style="display: none;">Update</span>
						<span class="button showHideBtn">Show</span>
						<span class="button cancelBtn" style="display: none;">Cancel</span>
						';
				...
				if ($roles == 'students') {
					if (key_exists('user_pin', $requred_title)) {
						echo '<div class="pass-control">';
						echo '<h2>Protected Results</h2>';
						echo educare_guide_for('Set protected password or pin code');
						...
						// user_pin
						echo '<p>'.esc_html( $requred_title['user_pin'] ).'</p>';
						...
						echo '
						<span class="button updatePasswordBtn">Update</span>
						<span class="button showHideBtn" style="display: inline-block;">Show</span>
						<span class="button cancelBtn" style="display: none;">Cancel</span>
						';
						echo '</div>';

						echo '<p>'.esc_html( $requred_title['user_pin'] ).' Status:</p>';
						...
						echo '<select name="default[pin_status]">
							<option value="valid" '.esc_attr(selected( $expire, 'valid', false )).'>Valid</option>
							<option value="expire" '.esc_attr(selected( $expire, 'expire', false )).'>Expire</option>
						</select>';
					}
				...
				// Extra fields
				echo '<h2>Others Info</h2>';
				echo educare_guide_for('add_extra_field');
				educare_get_extra_field();

				if ($roles == 'results' && educare_check_status('rattings_system') == 'checked') {
					echo '<h3 class="center">Rattings</h3>';
					echo educare_guide_for('You can <a href="/wp-admin/admin.php?page=educare-management&Rattings" target="_blank">Add More Ratting Fields</a> from Educare management menu. Also, you can <a href="/wp-admin/admin.php?page=educare-settings&menu=Results_System" target="_blank">Disable Ratings System</a> from Educare settings.');
							...
							if ($remarks) {
								...
								<thead>
									<tr>
										<th>No</th>
										<th>'.esc_html($term).'</th>
										<th class="rattings-status">Rattings</th>
									</tr>
								</thead>
								<tbody>
								';
								...
								foreach ($remarks as $remarks_field) {
									...
									echo '<tr>
										<td>'.esc_html($no++).'</td>
										<td>'.esc_html($remarks_field).'</td>
										<td><input type="number" value="'.esc_attr($value).'" name="Others[Rattings]['.esc_attr($term).']['.esc_attr($remarks_field).']" step="any"></td>
									</tr>';
								}
				...
				if ($roles == 'results' && educare_check_status('remarks') == 'checked') {
					...
					echo '<h2>Remarks</h2>';
					echo educare_guide_for('Click here to <a href="/wp-admin/admin.php?page=educare-settings&menu=Results_System" target="_blank">Disable Remarks System</a>.');

					// Master/Mistress Remarks
					echo '<p>Master/Mistress Remarks:</p>';
					echo '<input type="text" name="Others[remarks][teacher][comments]" value="'.esc_attr($remarks['teacher']['comments']).'" placeholder="Comments">';
					// Master/Mistress name
					echo '<input type="text" name="Others[remarks][teacher][name]" value="'.esc_attr($remarks['teacher']['name']).'" placeholder="Master/Mistress Name">';
					// remarks date
					echo '<input type="date" name="Others[remarks][teacher][date]" value="'.esc_attr($remarks['teacher']['date']).'" placeholder="Date">';
					// signature
					...
					<span class="button attachmentInput">Select Signature</span>
					<span class="button attachmentRemove">Remove</span>
					</p></div>';

					// principal remarks
					echo "<p>Principal's Remarks:</p>";
					echo '<input type="text" name="Others[remarks][principal][comments]" value="'.esc_attr($remarks['principal']['comments']).'" placeholder="Comments">';
					// Principal name
					echo '<input type="text" name="Others[remarks][principal][name]" value="'.esc_attr($remarks['principal']['name']).'" placeholder="Name of Principal">';
					// remarks date
					echo '<input type="date" name="Others[remarks][principal][date]" value="'.esc_attr($remarks['principal']['date']).'" placeholder="Date">';
					// signature
					echo '<div class="getAttachment">
					<p>Signature:';
					echo '<div class="attachmentPreview"><div class="attachmentImg">';
				...
				if ($roles == 'students' or $roles == 'results') {
					...
					echo educare_guide_for('If you need to select multiple subject, just press <code>Ctrl + Subject</code>');
				...
				echo '<br>';
				echo '<div class="select">';
				echo '<button type="submit" name="crud" class="educare_button crud-forms"><i class="dashicons dashicons-plus-alt"></i> '.esc_html($submit).'</button>';
				...
				if (isset($_POST['default']['id'])) {
					...
					echo '<button type="submit" name="view" formaction="'.esc_url($url).'" formtarget="_blank" class="educare_button"><i class="dashicons dashicons-visibility"></i> View</button>';
					echo '<button type="submit" name="delete" class="educare_button crud-forms"><i class="dashicons dashicons-trash"></i> Delete</button>';
					echo '<button type="button" class="educare_button" onclick="location.reload()"><i class="dashicons dashicons-plus-alt"></i> Add New</button>';
				}
