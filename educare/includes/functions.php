...
function educare_guide_for($guide, $details = null, $success = true) {
	if (educare_check_status('guide') == 'checked') {
		...
		if ($guide == 'add_extra_field') {
			$guide = "Click here to <a href='".esc_url($url . 'Extra_field')."' target='_blank'>Add More Fields</a>";
		}
		
		if ($guide == 'add_subject') {
			$guide = "Click here to <a href='".esc_url($url . 'Subject')."' target='_blank'>Add More Subject</a>. Also, you can disable, hide and control one or more of these marks term from <a href='/wp-admin/admin.php?page=educare-settings&menu=Card_Settings' target='_blank'>Card Settings</a>.";
		}
 ...
 * 
 */
 function educare_get_all_subject($options = false) {
...
function educare_files_selector($print) {
	...
	<div id='educare_files_selector_disabled'>
		<div id='educare_files_uploader' class='educare_upload add'>
			...
			<div class="select">
				<input id="educare_upload_button" type="button" class="button" value="Upload Students Photos">
 ...
 function educare_get_tab_management($action_for) {
	...
	if ($action_for == 'management') {
		...
		if (isset($_GET['import-data'])) {
			...
			educare_add_marks('marks');
		} else {
			echo '<h1>Marks List</h1>';
			educare_display_data('marks');
		...
		if (isset($_GET['import-data'])) {
			...
			echo educare_guide_for("Please select add (plus icon) button or <a href='".admin_url()."admin.php?page=educare-attendance&add-data'>click here</a> and choice your class, subject and date for update attendance");
		} elseif  (isset($_GET['add-data'])) {
			echo "<h1>Add Attendance</h1>";
			...
			educare_add_marks('attendance');
		} else {
			echo '<div class="cover"><img src="'.esc_url(EDUCARE_URL.'assets/img/marks.svg').'" alt="Attendance List" title="Add Attendance"/></div>';

			echo '<h1>Attendance List</h1>';
			// educare_display_data('attendance');
			echo educare_guide_for("Please select add (plus icon) button or <a href='".admin_url()."admin.php?page=educare-attendance&add-data'>click here</a> and choice your class, subject, date for add, edit, delete, print or view previously saved attendance data.");
			// educare_get_tab_management
		}
  ...
  $offset = ($page_no-1) * $per_page;
	...
	*/
	...
	if (isset($_POST['delete_all'])) {
		...
		if ($roles != 'results') {
			...
			if ($user_info) {
				...
				echo educare_show_msg('Based on your request: Total '.esc_html($total_user_info).' '.esc_html($roles).' (user) found. Successfully delete '.esc_html($count_success).' '.esc_html($roles).'. Error to delete '.esc_html($count_error).' '.esc_html($roles).'');
			} else {
				echo educare_show_msg('No user found to delete');
			}
		}
		// delete data
	...
	if (isset($_POST['update_status']) or isset($_POST['delete']) or isset($_POST['delete_all'])) {
		...
		if ( $process === false ) {
			// handle error
			echo educare_show_msg('There was an error processing your request', false);
		} elseif ( $process == 0 ) {
			...
			if (isset($_POST['update_status']) or isset($_POST['update_pin_status'])) {
				...
				echo educare_show_msg('No '.esc_html($roles).' found to delete');
			}
			...
			if (isset($_POST['update_status'])) {
				echo educare_show_msg('Successfully updated '.esc_html($roles).' status');
				// echo educare_show_msg('Successfully updated pin status');
			} elseif (isset($_POST['delete'])) {
				echo educare_show_msg('Successfully deleted ' . esc_html($roles));
			} else {
				echo educare_show_msg('Successfully deleted all ' . esc_html($roles));
			}
		}
	}
	...
	if ($requred_title) {
		...
		<form method="get" id="filter_data" class="add_results">
			...
			<div class="content">
				<div class="select add-subject">
					<div class="select">
						<div>
							<p id='Class_label'>Select <?php echo esc_html($requred_title['Class']);?>:</p>
						...
						<div>
							<p id='Exam_label'>Select <?php echo esc_html($requred_title['Exam']);?>:</p>
				...
				<div class="select">
					<div>
						<p>Select <?php echo esc_html($requred_title['Group']);?>:</p>
					...
					<div>
						<p>Select <?php echo esc_html($requred_title['Year']);?>:</p>
				...
				<div class="select">
					<div>
						...
						<select id='order_by' name="order_by">
							<option value='id' <?php esc_attr(selected( 'id', $order_by )) ;?>>Time</option>
							<option value='Name' <?php esc_attr(selected( 'Name', $order_by )) ;?>>Name</option>
							<option value='Roll_No' <?php esc_attr(selected( 'Roll_No', $order_by )) ;?>>Roll No</option>
							<option value='Regi_No' <?php esc_attr(selected( 'Regi_No', $order_by )) ;?>>Regi No</option>
						</select>
					</div>
					...
					<div>
						...
						<select id='select_order' name="order">
							<option value='DESC' <?php esc_attr(selected( 'DESC', $Status )) ;?>>Desc</option>
							<option value='ASC' <?php esc_attr(selected( 'ASC', $Status )) ;?>>Asc</option>
						</select>
					</div>
				</div>
				...
				<div class="select">
					<div>
						<p><?php echo esc_html($requred_title['user_pin']);?> Status:</p>
						<select id="pin_status" name="pin_status">
							<option value="">All</option>
							<option value="valid" <?php esc_attr(selected( 'valid', $pin_status )) ;?>>Valid</option>
							<option value="expire" <?php esc_attr(selected( 'expire', $pin_status )) ;?>>Expire</option>
						</select>
					</div>

					<div>
						<p>Change <?php echo esc_html($requred_title['user_pin']);?> Status To:</p>
						<select id="changepin_status" name="change_pin_status">
							<option value="">No Change</option>
							<option value="valid" <?php esc_attr(selected( 'valid', $change_pin_status )) ;?>>Valid</option>
							<option value="expire" <?php esc_attr(selected( 'expire', $change_pin_status )) ;?>>Expire</option>
						</select>
					</div>
				</div>
				...
				<div class="select">
					<div>
						...
						<select id="Status" name="Status">
							<option value="">All</option>
							<option value="publish" <?php esc_attr(selected( 'publish', $Status )) ;?>>Publish</option>
							<option value="scheduled" <?php esc_attr(selected( 'scheduled', $Status )) ;?>>Scheduled</option>
						</select>
					</div>
					...
					<div>
						...
						<select id="changeStatus" name="change_status">
							<option value="">No Change</option>
							<option value="publish" <?php esc_attr(selected( 'publish', $change_status )) ;?>>Publish</option>
							<option value="scheduled" <?php esc_attr(selected( 'scheduled', $change_status )) ;?>>Scheduled</option>
						</select>
					</div>
				</div>

				<input type="text" name="search" value="<?php echo $search?>" placeholder="Type here for search">

				<div class="button-container select">
					<button id="filter" type="submit" name="filter" class="educare_button" style="margin: 0;"><i class="dashicons dashicons-filter"></i> Filter</button>

					<button id="update_status" type="submit" name="update_status" class="educare_button" style="margin: 0;"><i class="dashicons dashicons-update"></i> Update Status</button>

					<button id="delete_all" type="submit" name="delete_all" class="educare_button" style="margin: 0;"><i class="dashicons dashicons-trash"></i> Delete</button>
				</div>
		...
		echo educare_show_msg('Search and filte system is not showing. Because, There has been a critical error on this website. Please go to Educare settings, then scroll down and click <b>Reset Settings</b> button.', false);
	}
		...
		<table class="view_results <?php echo esc_attr( $roles );?>">
			...
			<tbody>
				...
				if ($total_data) {
										...
										if ($default_check) {
											if (isset($print->$key) && $print->$key) {
												...
												if ($key == 'Group') {
													...
													echo "<td class='error'>none</td>";
													$results_button = 'error';
													$results_value = '&#xf530';
													$results_title = 'This '.esc_html( $roles ).' is not visible for users. Because, some required field are empty. Fill all the required field carefully. Otherwise, users getting arror notice when someone find this '.esc_html( $roles ).'. Click pen (Edit) button for fix this issue.';
												}
											}
										}
									}
								...
								<td>
									<div class="action_menu">
										...
										<menu class="action_link">
											...
											<?php
											// $link = admin_url();
											// $link .= 'admin.php?page=educare-all-'.$roles.'';
											...
											<form class="educare-modify" method="post" id="educare_results">
												...
												// View Button
												echo '<input class="button" type="submit" ' . esc_attr($results_button) . ' name="view" value="' . wp_check_invalid_utf8($results_value) . '" title="' . esc_attr(ucfirst($results_title)) . '" formaction="' . esc_url($url) . '" formtarget="_blank">';
												// Edit || Update button
												echo '<input class="button" type="submit" name="edit" value="&#xf464" title="Edit '.esc_attr( ucfirst($roles) ).'" formaction="'.esc_url($link).'&update-data" formtarget="_blank">';
												// Remove || Delete Button
												echo '<input class="button error" type="submit" name="delete" value="&#xf182" title="Delete '.esc_attr( ucfirst($roles) ).'">';
												?>
					...
					echo "<tr><td colspan='".esc_attr($col+2)."'><span class='error'>".esc_html($roles)." not found</span></td></tr>";
				}
			echo "</tbody>";
		echo "</table>";
	echo "</div>";
 ...
 * @return void|HTML
 */
function educare_add_marks($db = 'marks', $per_page = null) {
	global $wpdb;
	// Define table name to access data
	$roles = 'students';
	$table = $wpdb->prefix.EDUCARE_PREFIX.$roles;
	...
	// Define all empty fields to ignore php error
  $Year = $Class = $Group = $Exam = $Subject = $search = $Date = $order_by = $order = '';
	...
	*/
	...
	<form method="get" id="filter_data" class="add_results add_marks">
		...
		<div class="content">
			<div class="select">
				<div>
					<p>Select <?php echo esc_html($requred_title['Class']);?>:</p>
					<select id="Class" name="Class" class="form-control">
						<option value="">Select <?php echo esc_html($requred_title['Class']);?></option>
						<?php educare_get_options('Class', $Class);?>
					</select>
				</div>
			...
			if ($db == 'marks') {
				...
				<div class="select">
					<div>
						<p>Select <?php echo esc_html($requred_title['Exam']);?>:</p>
				...
				echo '<input type="date" name="Date" value="'.esc_attr($Date).'" placeholder="Select Date">';
			}
			?>
			...
			<div class="select">
				<div>
					<p>Order By:</p>
					<select id="order_by" name="order_by">
						<option value="id" <?php selected( esc_attr($order_by), 'id' );?>>Time</option>
						<option value="Name" <?php selected( esc_attr($order_by), 'Name' );?>><?php echo esc_html($requred_title['Name']);?></option>
						<option value="Roll_No" <?php selected( esc_attr($order_by), 'Roll_No' );?>><?php echo esc_html($requred_title['Roll_No']);?></option>
						<option value="Regi_No" <?php selected( esc_attr($order_by), 'Regi_No' );?>><?php echo esc_html($requred_title['Regi_No']);?></option>
						<option value="Group" <?php selected( esc_attr($order_by), 'Group' );?>><?php echo esc_html($requred_title['Group']);?></option>
					</select>
				</div>
				...
				<div>
					<p>Asc/Desc</p>
					<select id="select_order" name="order">
						<option value="DESC" <?php selected( esc_attr($order), 'DESC' );?>>Desc</option>
						<option value="ASC" <?php selected( esc_attr($order), 'ASC' );?>>Asc</option>
					</select>
				</div>
			</div>
	...
	if (isset($_GET['students_list'])) {
		...
		if (educare_is_empty($requred_fields)) {
			if (key_exists('Date', $requred_fields)) {
				echo educare_show_msg('You must fill <b>Attendance Date</b>.', false);
			} else {
			...
			if ($total_data) {
				...
				if ($opset_data) {
								...
								if ($marks_data) {
									foreach ($marks_data as $student_id => $subject) {
										...
										if ($student_data) {
											...
											if ($student_results) {
												// Results already exist. update data
												// echo '<pre>';
					...
					<div class="notice notice-success is-dismissible">
						<p>
							<b>'.esc_html($requred_title['Class']).':</b> '.esc_html($Class).'<br>
							<b>'.esc_html($requred_title['Group']).':</b> '.esc_html($display_group).'<br>
							<b>'.esc_html($requred_title['Exam']).':</b> '.esc_html($Exam).'<br>
							<b>Subject:</b> '.esc_html($display_sub).'<br>
							<b>'.esc_html($requred_title['Year']).':</b> '.esc_html($Year).'<br>
							<b>Students:</b> '.esc_html($over_all).'
						</p>
			...
			if (empty($target)) {
				...
				<div class="sticky_msg">
					<div class="notice notice-error is-dismissible"> 
						<p>You must fill the form for add the <b><?php echo esc_html($list);?></b>.</p>
						<button class='notice-dismiss'></button>
					</div>
				</div>
				<?php
		...
		if (isset($_POST['educare_edit_'.$list.''])) {
			...
			if ($in_list == 'Extra_field') {
				...
				<div class="sticky_msg">
					<div class="notice notice-success is-dismissible">
						<p>
							<h2 class="center">Edit <?php echo esc_html($list);?></h2>
							...
							<form action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>" method="post" class="add_results educare_update_data">
								...
								Selected - <b><?php echo esc_html($Target);?></b>
					...
					<div class="notice notice-success is-dismissible">
						<p>
							...
							<form action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>" method="post" class="add_results">
								...
								Selected - <b><?php echo esc_html($target);?></b><br>

								<p><?php echo esc_attr($list);?> Name:</p>
	...
	if ($search) {
		...
		if ($input) {
			if ($input === 'files') {
				...
				echo '<span class="button attachmentInput">Select Background</span>';
				echo '<span class="button attachmentRemove">Remove BG</span>';
				echo '</div></div>';
			...
			if ($target == 'display') {
				...
				if ($display) {
					foreach ($display as $key => $value) {
						...
						<div class="educare-settings">
							<div class="title">
								<h3><?php echo esc_html(ucwords(str_replace('_', ' ', $key))) . ' ' . wp_kses_post( $info );?><h3>
								<p class="comments">
									<input type='text' id='<?php echo esc_attr($key);?>' name='<?php echo esc_attr( $target . '[' . $key . ']');?>[title]' value='<?php echo esc_attr($field_name);?>' placeholder='Type <?php echo esc_attr($field_name);?>'>
								</p>
							</div>
							...
							<div class="status-button">
								<div class="switch-radio">
									<?php if ($key != 'Year') {
										?>
										<input type="radio" id="<?php echo esc_attr($target.'_'.$key);?>_no" name="<?php echo esc_attr( $target . '[' . $key . ']');?>[status]" value="unchecked" <?php if ($status == 'unchecked') { echo 'checked';};?>/>
										<label for="<?php echo esc_attr($target.'_'.$key);?>_no">No</label>
										<?php
									}?>
									
									<input type="radio" id="<?php echo esc_attr($target.'_'.$key);?>_yes" name="<?php echo esc_attr( $target . '[' . $key . ']');?>[status]" value="checked" <?php if ($status == 'checked') { echo 'checked';};?>/>
									<label for="<?php echo esc_attr($target.'_'.$key);?>_yes">Yes</label>
								</div>
							</div>
						</div>
						<?php
					...
					echo educare_show_msg('There has been a critical error on this website. Please scroll down and click <b>Reset Settings</b> button.', false);
				}
				...
				foreach ($display as $card_key => $card_value) {
					if ($card_key == 'details' or $card_key == 'grade_sheet') {
						...
						foreach ($card_value as $key => $value) {
							...
							if ($key == 'details') {
								$info = '<div class="action_menu"><i class="dashicons action_button dashicons-info"></i> <menu class="action_link info">Hide or show students '.esc_html($field_name).' header (h2) title. If you want to disable the entire details section from the card, please scroll down and uncheck <b>Students Details
								</b></menu></div>';
								...
								<div class="template-info-container fixbd-flex banner">
									...
									<div>
										<div>
											<img src="<?php echo esc_url( $data->banner->logo1 );?>">
											<input type="text" name="banner[logo1]" value="<?php echo esc_attr( $data->banner->logo1 );?>" placeholder="Photos URL (full path)">
										</div>
									</div>

									<div class="title">
										<div>
											<h1><input type="text" name="banner[title]" value="<?php echo esc_attr( $data->banner->title );?>" placeholder="Instutions name or slogan"></h1>
											<p class="sub-title"><input type="text" name="banner[subtitle1]" value="<?php echo esc_attr( $data->banner->subtitle1 );?>" placeholder="Place or something like this"></p>
											<p class="sub-title"><input type="text" name="banner[subtitle2]" value="<?php echo esc_attr( $data->banner->subtitle2 );?>" placeholder="Date or something like this"></p>
										</div>
									</div>

									<div>
										<div>
											<img src="<?php echo esc_url( $data->banner->logo2 );?>">
											<input type="text" name="banner[logo2]" value="<?php echo esc_attr( $data->banner->logo2 );?>" placeholder="Photos URL (full path)">
										</div>
									</div>
								</div>
								
								<!-- Details -->
								<h2 class="educare-title"><?php echo esc_html($field_name);?></h2>
								<div class="table_container">
									<table class="demo-table">
										<tbody>
											<tr>
												<td><?php echo esc_html($results_card['result']);?></td>
												<td><div class="success results_passed"><?php echo esc_html(educare_check_status('passed'));?></div></td>
												<td><?php echo esc_html($results_card['year']);?></td>
												<td>2022</td>
											</tr>
											<tr>
												<td><?php echo esc_html($results_card['gpa']);?></td><td>00</td>					
											</tr>
										</tbody>
									</table>
								</div>
							...
							if ($key == 'grade_sheet') {
								?>
								<h2 class="educare-title <?php echo esc_attr($status);?>"><?php echo esc_html($field_name);?></h2>
								<div class="table_container">
									<table class="grade_sheet demo-table">
										<tbody>
											<tr>
												<th class="<?php echo esc_attr($card_value->no->status);?>"><?php echo esc_html($results_card['no']);?></th>
												<th class="<?php echo esc_attr($card_value->subject->status);?>"><?php echo esc_html($results_card['subject']);?></th>
												<?php
												...
												if (isset($display->template->$current_template)) {
													...
													foreach ($display->template->$current_template as $card_field) {
														...
														if ($subtitle) {
															$subtitle = '<small>'.esc_html($subtitle).'</small>';
														}
														// show fields
														echo '<th class="'.esc_attr($card_field->status).'">'. esc_html($termTitle) . wp_kses_post( $subtitle ).'</th>';
													}
												}
												?>
											</tr>
								...
								<?php
								$info = '<div class="action_menu"><i class="dashicons action_button dashicons-info"></i> <menu class="action_link info">Hide or show students '.esc_html($field_name).' header (h2) title. If you want to disable the entire grade sheet section from the card, please scroll down and uncheck <b>Grade Sheet</b></menu></div>';
							}
							...
							<div class="educare-settings">
								<div class="title">
									<h3 class="educare-menu-container"><?php echo esc_html(ucwords(str_replace('_', ' ', $key))) . ' ' . wp_kses_post( $info );?><h3>
									<p class="comments">
										<input type='text' id='<?php echo esc_attr($key);?>' name='<?php echo esc_attr( $target . '[' . $card_key . ']' . '[' . $key . '][title]');?>' value='<?php echo esc_attr($field_name);?>' placeholder='Type <?php echo esc_attr($field_name);?>'>
									</p>
								</div>
								...
								<div class="status-button">
									<div class="switch-radio">
										<?php if ($key != 'Year') {
											?>
											<input type="radio" id="<?php echo esc_attr($target.'_'.$key);?>_no" name="<?php echo esc_attr( $target . '[' . $card_key . ']' . '[' . $key . '][status]');?>" value="unchecked" <?php esc_attr(checked( 'unchecked', $status ));?>>
											<label for="<?php echo esc_attr($target.'_'.$key);?>_no">No</label>
											<?php
										}?>
										
										<input type="radio" id="<?php echo esc_attr($target.'_'.$key);?>_yes" name="<?php echo esc_attr( $target . '[' . $card_key . ']' . '[' . $key . '][status]');?>" value="checked" <?php esc_attr(checked( 'checked', $status ));?>>
										<label for="<?php echo esc_attr($target.'_'.$key);?>_yes">Yes</label>
									</div>
								</div>
								...
								<div class="submenu">
									<p>Sub Titile:</p>
									<input type="text" name="<?php echo esc_attr( $target . '[' . $card_key . ']' . '[' . $key . '][subtitle]');?>" value="<?php echo esc_attr($value->subtitle);?>" palaceholder="Sub Titile">
								</div>
							</div>
							<?php
						}
					...
					if ($card_key == 'template') {
						...
						if (property_exists($card_value, $current_template)) {
							...
							foreach ($card_value->$current_template as $key => $value) {
								...
								<div class="educare-settings">
									<div class="title">
										<h3 class="educare-menu-container"><?php echo esc_html( $title );?></h3>
										<p class="comments">
											<input type="text" id="marks" name="<?php echo esc_attr( $target . '[' .$card_key. '][' .$current_template. '][' .$key. '][title]');?>" value="<?php echo esc_attr( $title );?>" placeholder="Type <?php echo esc_attr( $title );?>">
										</p>
									</div>
									...
									<div class="status-button">
										<div class="switch-radio">
											...
											<label for="<?php echo esc_attr($title.'_'.$key);?>_no">No</label>
											...
											<label for="<?php echo esc_attr($title.'_'.$key);?>_yes">Yes</label>
										</div>
									</div>

									<div class="submenu">
										<p>Sub Titile:</p>
										<input type="text" name="<?php echo esc_attr( $target . '[' .$card_key. '][' .$current_template. '][' .$key. '][subtitle]');?>" value="<?php echo esc_attr($subtitle);?>" palaceholder="Sub Titile">
										<p>Default value:</p>
										<input type="text" name="<?php echo esc_attr( $target . '[' .$card_key. '][' .$current_template. '][' .$key. '][default_value]');?>" value="<?php echo esc_attr($default_value);?>" palaceholder=">Default value">
										<p>Placeholder:</p>
										<input type="text" name="<?php echo esc_attr( $target . '[' .$card_key. '][' .$current_template. '][' .$key. '][placeholder]');?>" value="<?php echo esc_attr($placeholder);?>" palaceholder="Placeholder">
										
										<div class="select">
											<div>
												<p>Select Type:</p>
												<select name="<?php echo esc_attr( $target . '[' .$card_key. '][' .$current_template. '][' .$key. '][type]');?>">
													<option value="text" <?php esc_attr(selected( 'text', $type ));?>>Text</option>
													<option value="number" <?php esc_attr(selected( 'number', $type ));?>>Number</option>
													<option value="date" <?php esc_attr(selected( 'date', $type ));?>>Date</option>
													<option value="select" <?php esc_attr(selected( 'select', $type ));?> disabled>Options</option>
												</select>
											</div>
											...
											<div>
												...
												<select name="<?php echo esc_attr( $target . '[' .$card_key. '][' .$current_template. '][' .$key. '][auto]');?>">
													<option value="on" <?php esc_attr(selected( 'on', $auto ));?>>Yes</option>
													<option value="off" <?php esc_attr(selected( 'off', $auto ));?>>No</option>
												</select>
											</div>
											...
											<div>
												...
												<select name="<?php echo esc_attr( $target . '[' .$card_key. '][' .$current_template. '][' .$key. '][hide]');?>">
													<option value="on" <?php esc_attr(selected( 'on', $hide ));?>>Yes</option>
													<option value="off" <?php esc_attr(selected( 'off', $hide ));?>>No</option>
												</select>
											</div>
										</div>
									</div>
				...
				// for radio button
				if ($target == 'results_card_template' or $target == 'search_form_template' or $target == 'certificate_template') {
					// Show all template
					educare_show_template('educare_'.$target);

					if ($target == 'search_form_template' or $target == 'certificate_template') {
						...
						if ($template_data) {
							...
							foreach ($template_data as $field => $value) {
								...
								<p class='comments'>".esc_html($value['subtitle'])."</p>
		...
		<form class="educareUpdateSettings" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>" method="post">
			...
			<div class="collapses">
				<div class="collapse">
					...
					<label class="collapse-label" for="Shortcode_menu"><div><i class="dashicons dashicons-shortcode"></i> Shortcode</div></label>
					...
					<div class="collapse-content">
						...
						echo educare_guide_for("Inter your Front-End page slug (where you use educare shortcode in WP editor, template or any shortcode-ready area for front end results system). Don't need to insert with domain - <strike class='dissable'>".esc_url($domain)."/</strike><span class='success'><code>results</code></span>. Only slug will be accepted, for exp: <code>results</code> or <code>index.php/results</code>.");
						echo '</div>';

						echo '
						<div class="educare-settings"><div class="title"><h3>Results Shortcode</h3><h3>
						...
						<div class="educare-settings"><div class="title"><h3>Certificate Shortcode</h3><h3>
						<p class="comments">Copy and paste this <strong>'.esc_html($certificate_code).'</strong> shortcode in your editor, template or any shortcode-ready area for front end certificate system).</p>
						<input type="text" id="results_hortcode" value="'.esc_attr($certificate_code).'" disabled>
						</h3></div></div>

						<div class="educare-settings"><div class="title"><h3>Both (Results and Certificate)</h3><h3>
						<p class="comments">Copy and paste this <strong>'.esc_html($both_code).'</strong> shortcode in your editor, template or any shortcode-ready area for front end results and certificate system).</p>
						<input type="text" id="results_hortcode" value="'.esc_attr($both_code).'" disabled>
						</h3></div></div>

						<div class="educare-settings"><div class="title"><h3>Profiles Shortcode</h3><h3>
						<p class="comments">Copy and paste this <strong>`[educare_profiles]`</strong> shortcode in your editor, template or any shortcode-ready area for front end profile).</p>
						...
						<?php
						educare_settings_status('profiles_page', 'Profiles Page', "Inter your front end profiles page slug (where you use <strong>`[educare_profiles]`</strong> shortcode in your editor, template or any shortcode-ready area for front end profiles system). It's important when use educare login add-ons", true);
						?>
					</div>
				</div>
				...
				<div class="collapse">
					...
					<label class="collapse-label" for="Search_Form_menu"><div><i class="dashicons dashicons-search"></i> Search Form</div></label>
					<div class="collapse-content">
				...
				<div class="collapse">
					...
					<label class="collapse-label" for="Certificate_Card_menu"><div><i class="dashicons dashicons-id"></i> Certificate Card</div></label>
					<div class="collapse-content">
				...
				<div class="collapse">
					...
					<label class="collapse-label" for="Results_card_menu"><div><i class="dashicons dashicons-id"></i> Results Card</div></label>
					<div class="collapse-content">
					<div style='padding: 1px 0;'></div>
				...
				<div class="collapse">
					...
					<label class="collapse-label" for="Card_Settings_menu"><div><i class="dashicons dashicons-admin-appearance"></i> Card Settings</div></label>
					<div class="collapse-content" style="height: 100%;">
						...
						echo educare_show_msg('All options are performed based on the active template. So make sure, your active template supports these options.');
	...
	if ($search) {
		...
		if (isset($_POST["educare_process_$list"])) {
			...
			if (key_exists($class, $data)) {
				...
				if (empty($target)) {
					echo '<div class="sticky_msg"><div class="notice notice-error is-dismissible"> <p>You must fill the form for add the <b>'.esc_html($msgs).'</b>. thanks</p><button class="notice-dismiss"></button></div></div>';
				} else {
					...
					if ($search_terget) {
						...
						elseif (isset($_POST['edit_subject'])) {
							...
							<div class="sticky_msg">
								<div class="notice notice-success is-dismissible add_results">
									<p>
										<h2 class="center">Edit <?php echo esc_html(ucfirst($msgs));?></h2>
										...
										<form action="" method="post" class="add_results">
											...
											<div class="select add-subject">
												<div>
													<p><?php echo esc_html(ucfirst($msgs));?> name:</p>
												...
												<div>
													<p><?php echo esc_html(ucfirst($msgs));?> for <?php echo esc_html($list);?></p>
													<select name='class'>
														<?php
														foreach ($data as $key => $value) {
															if ($key == $class) {
						...
						elseif (isset($_POST['edit_class'])) {
							...
							<div class="sticky_msg">
								<div class="notice notice-success is-dismissible add_results">
									<p>
										<h2 class="center">Edit <?php echo esc_html(ucfirst($list));?></h2>

										<form action="" method="post" class="add_results">
											Selected - <b><?php echo esc_html($class);?></b>
						...
						elseif (isset($_POST['remove_subject'])) {
							$msg = '<div class="sticky_msg"><div class="notice notice-success is-dismissible"><p>Successfully removed <b>'.esc_html($target).'</b> from the '.esc_html($msgs).' list.</p><button class="notice-dismiss"></button></div></div>';
						...
						if (isset($_POST['add_subject'])) {
							...
							$msg = '<div class="sticky_msg"><div class="notice notice-success is-dismissible"> <p>Successfully Added <b>'.esc_html($target).'</b> at the '.esc_html( $msgs ).' list<br>'.esc_html( $list ).': <b>'.esc_html($class).'</b><br>Total: <b>'.esc_html(count($subject_list)).'</b> '.esc_html($msgs).' added</p><button class="notice-dismiss"></button>
							</div></div>';
						}
						...
						if (isset($_POST['update_subject'])) {
							...
							if (strtolower($target) != $old_subject and $class == $old_class) {
								...
								$msg = '<div class="sticky_msg"><div class="notice notice-success is-dismissible"> <p>Successfully change '.esc_html($msgs).' <b class="error">'.esc_html($old_sub).'</b> to <b class="success">'.esc_html($target).'</b></p><button class="notice-dismiss"></button></div></div>';
							}
							elseif (strtolower($target) == $old_subject and $class != $old_class) {
								...
								$msg = "<div class='sticky_msg'><div class='notice notice-success is-dismissible'><p>Succesfully update '.esc_html($msgs).' <b class='error'>".esc_html($old_sub)."</b> to <b class='success'>".esc_html($target)."</b>. also changed ".esc_html( $list )." <b class='error'>".esc_html($old_class)."</b> to <b class='success'>".esc_html($class)."</b>.</p><button class='notice-dismiss'></button></div></div>";
							}
						}
 ...
 function educare_setting_subject($list, $form = null) {
					...
					<div class="collapse-content bg-white">
						<table class='grade_sheet list'>
							<thead>
								<tr>
									<th>No</th>
									<th width='100%'><?php echo esc_html(ucfirst($msgs));?></th>
									<th>Edit</th>
									<th>Delete</th>
								</tr>
							</thead>
							...
							if ($val) {
								...
								echo "<tr><td colspan='4'><div class='notice notice-error is-dismissible'><p>Currently, you don't have added any ".esc_html($msgs)." in this ".esc_html( $list ).". Please add a ".esc_html($msgs)." for this ".esc_html( $list )." by using above forms. Thanks</p></div></td></tr>";
							}
							?>
							</tbody>
						</table>
	...
	if ($form) {
		...
		<div class="educare_tabs form_tab">
			<div class="tab_head">
				<button class="tablink educare_button" data="<?php echo esc_attr($list);?>_subject">Add <?php echo esc_html(ucfirst($msgs));?></button>
			...
			<div id="<?php echo esc_attr($list);?>_subject" class="section_name">
				<form class="add_results" action="" method="post" id="add_subject">
					<div class="content">
						...
						<div class="select add-subject">
							<div>
								<p><?php echo esc_html(ucfirst($msgs));?>:</p>
								<input type="text" name="subject" class="fields" placeholder="<?php echo esc_html(ucfirst($msgs));?> name" pattern="[A-Za-z0-9 ]+" title="Only characters, numbers and space allowed. (A-Za-z0-9)">
							</div>
						
							<div>
								<p><?php echo esc_html(ucfirst($msgs));?> For (<?php echo esc_attr($list);?>):</p>
						...
						<button id="educare_results_btn" class="educare_button proccess_<?php echo esc_attr($list);?>" name="add_subject" type="submit"><i class="dashicons dashicons-plus-alt"></i> Add <?php echo esc_html(ucfirst($msgs));?></button>
					</div>
				</form>
			</div>
 ...
 function educare_process_content() {
	...
	elseif (isset($_POST['educare_import_results'])) {
		...
		if (function_exists('educare_import_data')) {
			...
			echo educare_show_msg('You can not use this feature. Because, currently you have not installed <b>Educare Import and Export</b> add-ons. Please, contact your developers to install or enable this features.', false);
		}
 ...
 * @return mixed
 */
function educare_data_management($roles = null) {
	// Escape roles
	...
	if (isset($_GET['add-data'])) {
		...
		echo '<h1>Add '.esc_html($roles).'</h1>';

		echo educare_guide_for("Here you can add data and their details. Once, if you add and fill student details then you don't need to fill student details again while adding or publishing any result. If you miss something and need to update/edit, you can update a ".esc_html($roles)." details from the <a href='admin.php?page=educare-all-".esc_html($roles)."&update-data'>Update Menu</a>. Aslo, you can import unlimited ".esc_html($roles)." from <a href='admin.php?page=educare-all-".esc_html($roles)."&import-data'>Import</a> tab.");
		...
		// include (EDUCARE_ADMIN."menu/view-results.php");
		echo '<h1>Update '.esc_html($roles).'</h1>';

		echo educare_guide_for("Search ".esc_html($roles)." by roll, reg no, class and year for update or remove specific data (All fields are requred)");
		...
		// include (EDUCARE_ADMIN."menu/view-results.php");
		echo '<h1>Import '.esc_html($roles).'</h1>';
    ...
    echo '<div id="msgs">';
		...
		echo '<h1>All '.esc_html($roles).'</h1>';
		echo educare_guide_for("Here you can add, edit, update data and ".esc_html($roles)." details. For this you have to select the options that you see here. Options details: firt to last (All, Add, Update, Import ".esc_html(ucfirst($roles)).")");

		educare_display_data($roles);
	}
}
				...
				<form method='post' action="">
					<div class="educare_print">
						...
						echo "<div class='notice notice-success is-dismissible'><p>
							...
							<b>Group:</b> ".esc_html($Group)."<br>
							<b>Exam:</b> ".esc_html($Exam)."<br>
							<b>Subject:</b> ".esc_html($Subject)."<br>
							<b>Exam Year:</b> ".esc_html($Year)."<br>
							<b>Total Students:</b> ".esc_html(count($search_students))."
						</p><button class='notice-dismiss'></button></div>";

						echo '<input type="text" name="search" value="" placeholder="Type here for search" width="100%">';
					...
					<div class="button-container">
						...
						<div class="action_menu"><i class="dashicons action_button dashicons-info"></i> <menu class="action_link info"><strong>Mark not visible when print?</strong><br> Please, fill up students marks and save. Then, select <b>Students List</b> again and print marksheet (Save then Print).</menu></div>
					</div>
...
function educare_save_marks($publish = null) {
	// check if user access this request
	educare_check_access('mark_sheed');
		...
		if (!educare_is_empty($_POST)) {
			...
			if ($students) {
				...
				if ($promoted) {
					...
					$msgs = 'No students were found for promotion';
					$success = 'error';
				}
	...
	?>
		...
		<form class="add_results" action="" method="post">
			<div class="content">
				...
				<input type="checkbox" name="old_data" value="on" <?php checked($old_data, 'on' );?>> Keep old (Class) data
				<br><br>
...
function educare_show_template($template = 'educare_results_card_template') {
	...
	<div class="center">
		<b>'.esc_html(ucwords(str_replace('_', ' ', $template_for))).'</b>
		<br><small>You can find more template from <a href="https://fixbd.net" target="_blank">Here</a></small>
	</div><br>
			...
			echo '<label class="template-info-container" for="' . esc_attr($function_name) . '">
				...
				<div class="template-details">
					<div class="template-info">
						<b>'.esc_html( str_replace('_',  ' ', $template_title) ).'</b>
						<p>INSTALLED</p>
					</div>
				</div>
			</label>';
